...
sim <- list(nsamples = 100,  # same as mcElreath 
            ...
            pop_log_mean = mean(log(dataKline$log_pop)),
            pop_log_sd = sd(log(dataKline$log_pop)))
sim <- within(sim, {
  # define the model
  ...
  lst_log_s <- lapply(seq_len(nrow(grid)), FUN = function(i) {
    ...
    genData(n = nsamples, dtDefs = defs) %>%
      ...
      mutate(mean = grid$mean[i],
             sd = grid$sd[i],
             model = grid$model[i]) %>%
      expand(nesting(id, a, b, mean, sd, model),
...
p <- list()
p$pop_log_s <- ggplot(sim$data_log_s, aes(x = log_pop_s, y = lambda, group = id, color = model)) +
  geom_line() +
...
p$pop_log <- ggplot(sim$data_log, aes(x = log_pop, y = lambda, group = id, color = model)) +
  geom_line() +
  scale_fill_paletteer_d("LaCroixColoR::PassionFruit") +
...
p$hline <- 100
p$pop <- ggplot(sim$data_log, aes(x = exp(log_pop), y = lambda, group = id, color = model)) +
  geom_line() +
...
```{r}
p <- list()
p$pop_log_s <- ggplot(sim$lst_log_s[[2]], aes(x = log_pop_s, y = lambda, group = id)) +
  geom_line(color = "lightcoral") +
  theme(legend.position = "none") +
  labs(x = "Standard log population", y = "mean total tools(lambda)")
# p$pop_log_s

p$pop_log <- ggplot(sim$lst_log[[2]], aes(x = log_pop, y = lambda, group = id, color = model)) +
...
p$hline <- 100
p$pop <- ggplot(sim$lst_log[[2]], aes(x = exp(log_pop), y = lambda, group = id, color = model)) +
