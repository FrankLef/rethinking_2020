...
```{r}
ggplot(dataCareer, aes(x = factor(career), fill = factor(career))) +
...
# m11.13 <- rstan::stan(data = dat_list, model_code = code_m11.13, chains = 4)
...
`epred_draws` since the model has only the intercept.
...
```{r}
# stop("TODO")
getProbs <- function(x, inc = 0.5 * 1:3, coef = c(-2, 0, 2), digits = 3) {
  stopifnot(x >= 0, x <= 1)
  
  score <- inc + coef * x
  probs <- rethinking::softmax(score)
  stopifnot(sum(probs) == 1)
  out <- round(probs, digits = digits)
  
  diff <- 1 - sum(out)
  if (abs(diff) > 0) out[2] <- out[2] + diff
  stopifnot(sum(probs) == 1)
  out
}
# getProbs(runif(1))
...
sim <- within(sim, {
  defs <- defData(varname = "career", dist = "categorical",
                  formula = genCatFormula(getProbs(runif(1))))
  data <- genData(n = 500, dtDefs = defs)
