...
d <- chimpanzees %>%
  mutate(actor = factor(actor),
         treatment = factor(1 + prosoc_left + 2 * condition, levels = 1:4,
                            labels = c("a/r", "a/l", "p/r", "p/l"), ordered = TRUE))
rm(chimpanzees)
skimr::skim(d)
```
...
d %>%
  ...
  mutate(ldescription = c("Alone and two food items on the right",
...
ggplot(samples$data, aes(x = p, color = model)) +
  ...
  scale_color_manual(values = c("orange", "mediumseagreen")) +
...
Using `simstudy` is actually pretty simple
...
sim <- within(sim, {
  ...
  defs <- defData(defs, varname = "lnk1", dist = "nonrandom", formula = "a + b1",
                  link = "logit")
  defs <- defData(defs, varname = "lnk2", dist = "nonrandom", formula = "a + b2",
                  link = "logit")
  defs <- defData(defs, varname = "lnk3", dist = "nonrandom", formula = "a + b3",
                  link = "logit")
  defs <- defData(defs, varname = "lnk4", dist = "nonrandom", formula = "a + b4",
                  link = "logit")
  data1 <- genData(n = n, dtDefs = defs) %>%
    mutate(model = "sd = 10",
           p1 = inv_logit_scaled(lnk1),
           p2 = inv_logit_scaled(lnk2),
           p3 = inv_logit_scaled(lnk3),
           p4 = inv_logit_scaled(lnk4)) %>%
    mutate(diff = abs(p1 - p2))
...
```{r}
ggplot(d, aes(x = as.factor(career))) +
  geom_bar(aes(y=..count.. / sum(..count..)), fill = "wheat") +
  ggthemes::theme_few() +
  labs(x = "career")
```
