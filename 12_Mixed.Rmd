...
In the `dgamma` the shape parameter influence the rate which is equivalent to
Poisson $lambda$
...
```{r}
a_file <- here::here("fits", "m12_02a.rds")
m12.2a <- readRDS(file = a_file)
# m12.2a <- ulam(
#   data = dataKline,
#   flist = alist(
...
#   log_lik = TRUE
# )
# saveRDS(object = m12.2a, file = a_file)
```
...
one value for the Intercept which is the mean of the 10 Poisson rates 
$\lambda_i, i =1,...10$.
...
And the prediction plots show that the distributions all use the same rate and 
shape.
...
We observe that $lambda$ matches the actual rate of our simulation with
...
p$freq <- ggplot(p$df, aes(x = response, y = n, fill = response)) +
  ...
  scale_fill_paletteer_d(palette = "khroma::discrete_rainbow") +
  theme(legend.position = "none") +
...
and to validate our fit, we see that the  $value$ in the summary is the same
as the $cum_pct$ previously computed.
...
> This form automatically ensure the correct ordering of the outcome values,
