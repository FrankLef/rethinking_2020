...
The intercept represents $\lambda$ on the log scale, because the link function
for $\lambda$. This can be confirm by looking at the summary which shows
**Links: mu = log; zi = identity**.

We observe that this is the actual rate of our simulaiton with
`defData(defs, varname = "work", dist = "poisson", formula = 1)`.

```{r}
fixef(b12.3)
exp(fixef(b12.3)[, "Estimate"])
```
...
We observe that this is the actual rate of our simulation with
