...
# NOTE: inlcue dept for predicted_draws to work ! i.e. all factors
# even when not used
samples$newdata <- dataAdmit %>%
  distinct(dept, gid, applications, admit)
...
$$
```{r}
prm <- gammaGetShapeRate(mean = 5, dispersion = 1.5)
var <- 5^2 * 1.5
1 / var
prm
```
...
```{r}
p <- list()
p$df <- crossing(shape = c(0.1, 0.5, 1, 2), rate = c(1/10, 1/5, 1/2, 1)) %>%
...
p$plot <- ggplot(data = p$df %>% filter(shape == 0.1, rate == 0.1)) +
  geom_area(aes(x = x, y = density),
              fill = "maroon1") +
  scale_y_continuous(NULL, labels = NULL) +
  labs(title = "Gamma ptior with default (shape = 0.1, rate = 0.1)",
       x = "parameter space") +
  ggthemes::theme_hc() +
  theme(axis.ticks.y = element_blank(),
        plot.background = element_rect(fill = "grey92"))
...
```{r}
p$plot <- ggplot(data = p$df) +
  geom_area(aes(x = x, y = density), fill = "maroon1") +
  scale_y_continuous(NULL, labels = NULL) +
  ...
  theme(axis.ticks.y = element_blank(),
        plot.background = element_rect(fill = "grey92")) +
  facet_grid(shape_char~rate_char, labeller = label_parsed)
p$plot
```
...
Lets do it the right way.  Start with the null model, or as Kurtz calls it,
