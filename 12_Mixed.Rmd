...
b12.6 <- readRDS(file = a_file)  # load the file
# b12.6 <-
#   brm(data = d,
#       family = negbinomial,
#       admit ~ 1,
...
This is an extra section.  The result is so nice it is worth adding here. See
...
```{r}
(dordlogit.new(fixef(b12.1)[, 1]))
```

```{r}
fixef(b12.1)[, 1]
stats::plogis(q = fixef(b12.1)[, 1], lower.tail = TRUE)
stats::plogis(q = fixef(b12.1)[, 1], lower.tail = FALSE)
stats::plogis(q = fixef(b12.1)[, 1])
stats::dlogis(x = fixef(b12.1)[, 1])
```
...
```{r}
sum(dordlogit.new(fixef(b12.1)[, 1]) * 1:7)
```

but if we substract 0.5

```{r}
(dordlogit.new(fixef(b12.1)[, 1], phi = 0.5))
```
then we have a higher expected value

```{r}
sum(dordlogit.new(fixef(b12.1)[, 1], phi = 0.5) * 1:7)
```
...
```{r}
loo_compare(b12.1, b12.2, b12.3, criterion = "waic")
```
