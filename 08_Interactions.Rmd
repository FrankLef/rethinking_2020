...
ggplot(dd, aes(x = rugged_sc, y = log_gdp_s, fill = is_africa, color = is_africa)) +
  ...
  labs(title = "Figure 8.4", subtitle = "model b8.2")
```
...
ggplot(dd, aes(x = rugged_sc, y = log_gdp_s, fill = is_africa, color = is_africa)) +
  ...
  labs(title = "Figure 8.5", subtitle = "model b8.3") +
...
$$
\begin{align*}
blooms\_r &= \mathcal{N}(\mu_i, \sigma) \\
...
```
lets say that we we wnat only 5% of the values outside the range (2.5% on each side)
...
ggplot(df, aes(x = water_c, y = blooms_r)) +
  geom_line(aes(group = row),
            color = "steelblue", alpha = 1/5, size = 1/2) +
